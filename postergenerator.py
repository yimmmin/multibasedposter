...
def sourcepic_formalblock(formalsize=100, blocksize=5):
    ...
    i = 1
    if not os.path.exists('images_formalized'):
        os.mkdir('images_formalized')
    if not os.path.exists('images_blocken'):
        os.mkdir('images_blocken')
...
def targetpic_match(mergex=100, mergey=100, blocksize=5, pic_num=100):
    ...
    for x in range(0, blockx):
        for y in range(0, blocky):
            ...
            print(str(round(n/blockx/blocky*100,2))+'% positions have been chosen.')
...
def poster_generator(mergex = 100, mergey = 100, blocksize = 5):
    ...
    n = sourcepic_formalblock(150,blocksize)
